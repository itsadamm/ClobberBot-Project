...
/**************************************************************************************************/
    ...
    private class RandomSeedArg extends ArgHandler
    {
        public RandomSeedArg() { arg="-seed"; }

        public int handleArg(String[] args, int x) throws Exception
        {
            if(argMatch(args[x]))
            {
                seed=Long.parseLong(args[++x]);
                return ++x;
            }
            else return x;
        }
...
/**************************************************************************************************/
    ...
    private void initArgHandlers()
    {
        ...
        argHandlers.add(new RandomSeedArg());
    }
    ...
    private void tallyResults()
    {
        for(int x=0;x<bots.size();x++)
        {
            ...
            bots.get(x).res.points += (bots.get(x).numKills * 1);
            bots.get(x).res.points += 10;
...
/**************************************************************************************************/
    ...
    class MyClassLoader extends ClassLoader {
       ...
       {
             System.out.println("loading classes = " + classname);System.out.flush();
             for(String fname:classname.split(" "))
             {
                 System.out.println("classname = " + fname);System.out.flush();
                 ...
                 Class foo = defineClass (fname.substring(0,fname.length()-6), rec, 0, rec.length);
                 if(cls==null) cls = foo;
             }
       }
    }
    ...
    private void loadPlayerFromFile(Vector<ClobberBot> bots, String fname) throws Exception
    {
        ...
        /*Class<?>[] foo = cls.getDeclaredClasses();
        for(Class f:foo)
        {
    ...
    private class ClobberTourney
    {
        ...
        private Vector<ClobberBotRes> loadSection1Bots()
        {
            ...
            //clobberbots.add(new ClobberBotRes(new p10(dummy)));
            //clobberbots.add(new ClobberBotRes(new p12(dummy)));
            clobberbots.add(new ClobberBotRes(new ExperimentBot(dummy)));
        ...
        {
            String str="BOT\t\t\t\tPoints\tnumDied\tnumKills\tnumSurvived\texceptions\n";
            for(int x=0;x<bots.size();x++)
...
/**************************************************************************************************/
    public static void main(String[] args) throws Exception
    {
        ...
        else 
        {
            ...
            for(ClobberBot bot:bots)
			{
			    bot.rand.setSeed(game.rand.nextLong());
			}
